    ...
    def register_tools(self):
        ...
        async def toggle_visibility(            
            ...
            try:
                ...
                if not token:
                    return UserProfileResponse(success=False, data=None, message="❌ Authentication required. Please sign in to toggle visibility.")
                ...
                if "user" not in result:
                    return UserProfileResponse(success=False, data=None, message="❌ Unable to update visibility. Please try again later.")  # type: ignore

                user_data = result["user"]
                validated_response = UserProfile.model_validate(user_data)

                visibility_status = "visible" if validated_response.visible else "hidden"
                return UserProfileResponse(
                    success=True, 
                    data=validated_response,
                    message=f"✓ Your profile is now {visibility_status}"
                )
        ...
        async def get_user_profile() -> UserProfileResponse:
            ...
            try:
                ...
                if "user" not in result:
                    return UserProfileResponse(success=False, data=None, message="❌ Unable to retrieve your profile. Please try again later.")  # type: ignore
