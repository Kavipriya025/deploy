...
# Default expiry window when the user does not provide one
...
{{
    ...
    def _create_extraction_result(self, user_input: str, llm_result: Dict[str, Any]) -> ExtractedWhistleData:
        ...
        if expiry == "default":
            expiry_date = datetime.now() + timedelta(hours=self.default_expiry_hours)
    ...
    def validate_whistle_data(data: ExtractedWhistleData) -> Dict[str, Any]:
        ...
        if data.expiry != "never":
            try:
                ...
                default_expiry = datetime.now() + timedelta(hours=DEFAULT_EXPIRY_HOURS)
        ...
        async def create_whistle(
            ...
            try:                
                ...
                if needs_clarification:
                    clarification_message = extracted_data.reason or "; ".join(validation_result["errors"])
                    suggestions = self._generate_dynamic_suggestions(extracted_data, validation_result)
                    
                    # Build user-friendly message
                    user_message = f"ℹ️ {clarification_message}"
                    if suggestions:
                ...
                logger.info("whistle_data",whistle_data)
                token: AccessToken | None = get_access_token()

                if not token:
                    return {
                        "status": ProcessingStatus.ERROR.value,
                        "message": "❌ Authentication required. Please sign in to create a whistle."
                    }
                ...
                if not new_whistle:
                    return {
                        "status": ProcessingStatus.ERROR.value,
                        "message": "❌ Unable to create whistle. Please try again later."
                    }
                ...
                # Get matching whistles from API response if available
                ...
                if "ETLIMIT" in error_msg:
                    return {
                        "status": ProcessingStatus.ERROR.value,
                        "message": "❌ Too many tags specified. Please use fewer tags (maximum 20 allowed)."
                    }
        ...
        async def list_whistles(
            ...
            try:            
                ...
                if not token:
                    return {
                        "status": ProcessingStatus.ERROR.value,
                        "message": "❌ Authentication required. Please sign in to list your whistles."
                    }
               ...
               # Fetch user details from the 'user' endpoint
                ...
                count = len(formatted_whistles)
                filter_text = "active " if active_only else ""
                message = f"✓ Found {count} {filter_text}whistle{'s' if count != 1 else ''}"
                ...
                return {
                    "status": "error",
                    "message": "❌ An unexpected error occurred while listing your whistles. Please try again later.",
                    "whistles": []
                }
